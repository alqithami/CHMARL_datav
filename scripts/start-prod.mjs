...
process.env.DATALASTIC_MAX_VESSELS = "0";
...
console.log("Vessel input policy: genuine AIS only; all valid global coordinates are retained without an application vessel-count ceiling. AISStream is primary, Datalastic is optional port coverage, and PocketWorld is continuously merged as a public regional source. Manual, fixed, sample, and synthetic vessel rows are disabled.");
if (process.env.AISSTREAM_API_KEY?.trim()) console.log("AISStream API key loaded from environment.");
if (process.env.DATALASTIC_API_KEY?.trim()) console.log("Datalastic live AIS key loaded from environment.");
else console.log("Datalastic live AIS key is not configured.");
console.log(`PocketWorld public AIS source: ${process.env.POCKETWORLD_AIS_ENABLED}; aggregate count policy unlimited; display retention ${process.env.POCKETWORLD_DISPLAY_MAX_AGE_MS} ms; fresh threshold ${process.env.POCKETWORLD_FRESH_AGE_MS} ms.`);
