...
function sortedData(data: PortUtilizationChartProps["data"]) {
...
function utilizationSummary(rows: PortUtilizationChartProps["data"]) {
  if (rows.length === 0) {
    return "The port utilization feed is not connected.";
  }
  return rows
    .map(
      (row) =>
        `${row.name}: ${row.value}% utilization, ${pressureLabel(row.value)} state`
    )
    .join("; ");
}
...
export default function PortUtilizationChart({
  ...
  const summary = useMemo(() => utilizationSummary(rows), [rows]);
      ...
      tooltip: {
        ...
        formatter: (items: unknown) => {
          const item = Array.isArray(items)
            ? (items[0] as { name: string; value: number })
  ...
  return (
    <Chart
      option={option}
      ariaLabel="Port utilization chart"
      summary={summary}
    />
  );
}
