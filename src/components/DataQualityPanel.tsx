...
function vesselStatus(data: DashboardData): QualityItem {
  if (data.source === "aisstream") {
    return {
      label: "AIS",
      value: "live",
      detail: `${data.vessels.length} current vessel rows`,
  ...
  if (data.source === "aisstream-waiting") {
    return {
      label: "AIS",
      value: "waiting",
      detail: "socket connected; waiting for positions in monitored boxes",
      tone: "warn",
    };
  }
  if (data.source === "upstream" || data.source === "remote") {
    return {
      label: "Vessels",
      value: "provider",
      detail: `${data.vessels.length} rows from backend provider`,
      tone: data.vessels.length > 0 ? "good" : "warn",
    };
  }
  if (data.source === "local-json") {
    return {
      label: "Vessels",
      value: "sample",
      detail: "local fixture data is enabled",
      tone: "warn",
    };
  }
  return {
    label: "Vessels",
    value: "missing",
    detail: "no live AIS/provider rows available",
    tone: "missing",
  };
}
...
function chmarlStatus(data: DashboardData): QualityItem {
  if (data.chmarlSource === "runtime") {
    return {
      ...
      detail: data.chmarlExperimentId ?? "derived from active operational feeds",
      tone: "good",
    };
  }
  if (data.chmarlSource === "local-json") {
    return {
      label: "CH-MARL",
      value: "sample",
      detail: "demo episode is enabled",
      tone: "warn",
    };
  }
  return {
    label: "CH-MARL",
    value: "inactive",
    detail: "requires live AIS rows or a runtime experiment feed",
    tone: "missing",
  };
}
...
function portStatus(data: DashboardData): QualityItem {
  if (data.portOpsSource === "runtime") {
    return {
      label: "Port ops",
      value: "provider",
      detail: `${data.portEvents.length} events · ${data.portUtilization.length} utilization rows`,
      tone: "good",
    };
  }
  if (data.portOpsSource === "local-json") {
    return {
      label: "Port ops",
      value: "sample",
      detail: "local port fixture is enabled",
      tone: "warn",
    };
  }
  return {
    label: "Port ops",
    value: "required",
    detail: "connect PORT_EVENTS_URL for berth/queue/utilization",
    tone: "missing",
  };
}
...
function weatherStatus(data: DashboardData): QualityItem {
  if (data.weatherSource === "open-meteo") {
    return {
      label: "Weather",
      value: "backend Open-Meteo",
      detail: `${data.weatherPoints.length} marine points`,
  ...
  return {
    label: "Weather",
    value: "missing",
    detail: "backend weather feed unavailable",
    tone: "missing",
  };
}
