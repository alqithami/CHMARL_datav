...
const AISSTREAM_BBOX = process.env.AISSTREAM_USE_SAUDI_PORT_BBOXES === "false"
  ? (process.env.AISSTREAM_BBOX ?? SAUDI_PORT_BBOX)
  : SAUDI_PORT_BBOX;
...
const chmarlState = { source: CHMARL_EXPERIMENT_URL ? "url" : "file", configuredUrl: Boolean(CHMARL_EXPERIMENT_URL), file: CHMARL_EXPERIMENT_FILE, steps: 0, experimentId: null, scenarioId: null, lastLoadedAt: null, lastError: null };
const portOpsState = { source: PORT_EVENTS_URL ? "url" : "file", configuredUrl: Boolean(PORT_EVENTS_URL), file: PORT_EVENTS_FILE, events: 0, utilizationRows: 0, queueRows: 0, lastLoadedAt: null, lastError: null };
const weatherState = { source: WEATHER_URL ? "url" : existsSync(WEATHER_FILE_PATH) ? "file" : "open-meteo", configuredUrl: Boolean(WEATHER_URL), file: WEATHER_FILE, points: 0, lastLoadedAt: null, lastError: null };
...
function messageBody(message) {
  ...
  const body = message?.Message?.[type] ?? message?.Message?.PositionReport ?? message?.Message?.StandardClassBPositionReport ?? message?.Message?.ExtendedClassBPositionReport;
...
function loadAisCacheFromDisk() {
  ...
  try {
    ...
    for (const row of rows.slice(0, AISSTREAM_MAX_VESSELS)) {
      if (row?.id) aisCache.set(String(row.id), normalizeVessel(row));
    }
    aisState.cachedVessels = aisCache.size;
...
async function loadVessels() {
  ...
  if (UPSTREAM_URL) {
    ...
    const rows = rowsFrom(payload, ["vessels", "data", "items"]);
...
async function loadChmarlExperiment() {
  try {
    ...
    if (existsSync(CHMARL_EXPERIMENT_FILE_PATH)) return updateChmarlState(JSON.parse(readFileSync(CHMARL_EXPERIMENT_FILE_PATH, "utf8")), "file");
    chmarlState.steps = 0;
    chmarlState.lastError = null;
    return null;
  } catch (error) {
...
async function loadPortOperations() {
  try {
    ...
    if (existsSync(PORT_EVENTS_FILE_PATH)) return updatePortOpsState(JSON.parse(readFileSync(PORT_EVENTS_FILE_PATH, "utf8")), "file");
...
async function loadOpenMeteoPoint(point) {
  ...
  return {
    ...
    currentVelocityMs: hourlyValue(hourly, "ocean_current_velocity", index),
    currentDirectionDeg: hourlyValue(hourly, "ocean_current_direction", index),
...
async function loadWeather() {
  try {
    ...
    if (existsSync(WEATHER_FILE_PATH)) {
...
function rootHtml(staticAvailable) {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8" /><meta name="viewport" content="width=device-width, initial-scale=1" /><title>CH-MARL Backend</title></head><body style="font-family:system-ui;background:#04111f;color:#e6f7ff;display:grid;place-items:center;min-height:100vh"><main><h1>CH-MARL backend service</h1><p>${staticAvailable ? "Serving production dashboard." : "Use Vite port 5173 in development."}</p><p>Endpoints: /health, /api/vessels, /api/port-events, /api/chmarl/episode, /api/weather</p></main></body></html>`;
}
...
createServer(async (request, response) => {
  ...
  if (request.url === "/api/vessels") {
    try {
      ...
      return sendJson(response, 200, { ...result, health: { ...aisState, cachedVessels: aisCache.size } });
    } catch (error) {
  ...
  if (request.url === "/" || request.url === "") return sendJson(response, 200, { service: "chmarl-backend", endpoints: ["/health", "/api/vessels", "/api/chmarl/episode", "/api/port-events", "/api/weather"] });
  return sendJson(response, 404, { error: "Not found", availableEndpoints: ["/", "/health", "/api/vessels", "/api/chmarl/episode", "/api/port-events", "/api/weather"] });
}).listen(PORT, () => {
